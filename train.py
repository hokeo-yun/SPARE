...
if __name__ == '__main__':
    ...
    for epoch in range(1, opt.niter + 1):
        ...
        for i, data in enumerate(data_loader):
            ...
            print(
                f"\r[Epoch {epoch:02d}/{opt.niter:02d} | Batch {i + 1:04d}/{len(data_loader):04d} | Time {training_time + time.time() - start_time:1.1f}s] loss: {model.loss.item():1.7f}",
                end="",
            )
        ...
        if acc > mx_acc :
            model.save_networks( f'best_model.pth')
            mx_acc = acc

        early_stopping(acc, model)
        if early_stopping.early_stop:
            cont_train = model.adjust_learning_rate(epoch, opt.niter)
            if cont_train:
